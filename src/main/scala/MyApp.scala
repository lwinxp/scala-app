...
//  override def port: Int = 8000 // avoid clash 8080
  ...
  def submit(request: cask.Request): Unit = // there is request but no response, just writing to DB next line
    val todoInsertStmt = DbDetails.conn.prepareStatement("""insert into todo values(?)""") // connection in file DbDetails.scala
  ...
  def readtodos(): String =
    val readStmt = DbDetails.conn.prepareStatement("""select item from todo""") // item is the only thing in the table, item is a string, otherwise none
    ...
    val todos = Iterator.continually { // use iterator continually
      if qResults.next() then Option(qResults.getString("item")) else None // normally when have iterator, iterator is already on 1st elem. but here iterator is before 1st elem, need to call qResults.next() first
